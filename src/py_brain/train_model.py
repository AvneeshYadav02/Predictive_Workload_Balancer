...
#  TIME = (Coef_1 x Task_ID) + (Coef_2 x Input_Data) + Intercept
# To get an estimated time in our final steps in C we will directly use these coefficients and the intercepts.
print(f"Formula: ({model.coef_[0]:.4f} * Task_ID) + ({model.coef_[1]:.4f} * Input_Data) + ({model.intercept_:.4f})")